...
public class HNLMNSpiderTaskService {
	// 河南
	public void getHNLMNLandDetail(LMNLandParams lMNLandParams, List<LMNLandMessageDetail> lMNLandMessageDetailList,
			int num) throws IndexOutOfBoundsException, IOException {
		String urlOne = "http://www.hngtjy.cn/home.jsp";
		// 模拟浏览器操作
		// 创建WebClient
		// 注意浏览器的选择，此网站只支持ie且是7或8
		...
		HtmlElement hee = page.getHtmlElementById("cantons-channel-btn");
		page = hee.click();
		page = page.getElementById("Td0").getElementsByTagName("a").get(0).click();
		// 获取第一页的六个土地代码
		List<String> landCodeList = getHNLandCode(num);
		// 遍历
		for (String landCode : landCodeList) {
			...
			for (DomElement temp : he) {
				if (temp.getAttribute("class").equals("td_line2")) {
					...
					// 传递table循环取数据
					isSuccess = setHNLandMeg(lMNLandMessageDetailList, ht, landCode);
				}
			}
		}
	...
	// 获取关键码,相当于每个公告有个代码
	// 河南
	private List<String> getHNLandCode(int pageNum)
			throws FailingHttpStatusCodeException, MalformedURLException, IOException {
		List<String> landCodeList = new ArrayList<String>();
		String url = "http://www.hngtjy.cn/deala_js_action?resourcelb=01&dealtype=&JYLB=&JYFS=&JYZT=&RESOURCENO=&RESOURCEMC=&ZYWZ=&currentPage="
				+ pageNum;
		// 模拟浏览器操作
		// 创建WebClient
		WebClient webClient = new WebClient(BrowserVersion.EDGE);
		...
		for (DomElement temp : he) {
			if (temp.getAttribute("class").equals("list1_btn1")) {
				...
				ss = s.substring(s.indexOf("goRes('") + 7, s.indexOf("goRes('") + 11);
				landCodeList.add(ss);
				// 5200
			}
		}
	...
	// 取数据
	// 河南
	private boolean setHNLandMeg(List<LMNLandMessageDetail> lMNLandMessageDetailList, HtmlTable ht, String landCode)
		...
		for (int i = 0; i < rowNum; i++) {
			...
			for (int j = 0; j < row.getCells().size(); j = j + 2) {
				...
				setHNContentByLabel(label, content, lMNLandMessageDetail);
			}
			row = null;
			label = "";
			content = "";
		...
		// 尚未找到好办法
		getHNOtherLandDetail(lMNLandMessageDetail, landCode);
	...
	// 填充信息
	// 浙江
	private void setHNContentByLabel(String label, String content, LMNLandMessageDetail lMNLandMessageDetail) {
	...
	// 获取其他信息
	// 浙江
	private void getHNOtherLandDetail(LMNLandMessageDetail lMNLandMessageDetail, String landCode)
			throws FailingHttpStatusCodeException, MalformedURLException, IOException {
	...
	// 筛选
	// 浙江
	public List<LMNLandMessageDetail> checkHNLandList(List<LMNLandMessageDetail> jsLandList,
