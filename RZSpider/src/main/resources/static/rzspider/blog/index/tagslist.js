$(document)
		.ready(
				function() {
					title = basicsetTitle + "-标签云（" + blogtags.blogTagsName
							+ "）";
					// 设置标签信息
					if (blogtags != null && blogtags != '') {
						...
						$(".sh")
								.css(
										"background",
										'url('
												+ ((blogtags.backImg == null || blogtags.backImg == '') ? bgImg
														: blogtags.backImg)
												+ ') no-repeat');
						// 动态设置css实现拉伸效果
						...
						document.getElementById("n1").innerText = "标签云";

						document.getElementById("n2").href = '/rzblog/blogtags/?tags='
								+ blogtags.blogTagsName;
						document.getElementById("n2").innerText = blogtags.blogTagsName;

					}
					document.title = title;

					// 设置寄语
					document.getElementById("tagsMsg").innerHTML = blogtags.tagsMessage;
				})
...
function bindList() {
	...
	$
			.ajax({
				...
				data : {
					status : 1,
					blogTagsName : blogtags.blogTagsName,
					pageSize : pageSize,
					pageNum : pageNum,
				...
				success : function(data) {
					var rows = data.rows;
					if (rows == null || rows == '') {
						var nodataHtml = '<p class="news_title">暂无文章，请浏览其它标签！</p>';
					...
					for (i = 0; i < rows.length; i++) {
						...
						htmlText = '<div class="blogs" style="background-color:'
								...
								+ rows[i].gtmCreate
								+ '</li>'
								+ '<li class="view"><span><span><i class="fa fa-eye"></i> '
								+ rows[i].browseNum
								+ '</span></li>'
