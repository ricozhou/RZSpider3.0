...
$(function() {
	...
	editor.setValue($('#spiderJavaCodeExample').val());
...
// 表单验证
$("#form-spiderJavaCodeExample-edit").validate({
	rules : {
		// 此验证不成功需要重新验证
		code : {
			required : true,
...
function update() {
	...
	var spiderJavaCodeExample;
	if (radio1) { 
		spiderJavaCodeExample = $.trim(editor.getValue());
	} else {
		// 文件上传
		if (codeString == null) {
	...
	$.ajax({
		...
		data : {
			"basesetId" : basesetId,
			"spiderJavaCodeExample" : spiderJavaCodeExample
		},
		async : false,
