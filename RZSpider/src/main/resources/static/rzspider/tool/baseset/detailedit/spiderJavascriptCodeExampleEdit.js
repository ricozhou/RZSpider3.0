...
$(function() {
	...
	editor.setValue($('#spiderJavascriptCodeExample').val());
...
// 表单验证
$("#form-spiderJavascriptCodeExample-edit").validate({
	rules : {
		// 此验证不成功需要重新验证
		code : {
			required : true,
...
function update() {
	...
	var spiderJavascriptCodeExample;
	if (radio1) {
		spiderJavascriptCodeExample = $.trim(editor.getValue());
	} else {
		// 文件上传
		if (codeString == null) {
	...
	$.ajax({
		...
		data : {
			"basesetId" : basesetId,
			"spiderJavascriptCodeExample" : spiderJavascriptCodeExample
		},
		async : false,
