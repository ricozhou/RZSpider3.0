		...
		callback : {
			...
			onDblClick : function(event, treeId, treeNode) {
				...
				if (treeNode.getParentNode() != null
						...
						&& treeNode.getParentNode().getParentNode()
								.getParentNode() == null
						&& treeNode.getParentNode().getParentNode().name == customSpiderBackId
						&& treeNode.name.endWith(".js")) {
...
function showRMenu(type, x, y, treeNode) {
	...
	if (treeNode.name == 'publiclib' && treeNode.getParentNode() == null) {
		// 可上传，下载
		$("#installPackage").css('display', 'block');
		// 其他禁用
		...
		$("#deleteFile").css('display', 'none');
			...
			&& treeNode.getParentNode().getParentNode() == null
			&& treeNode.getParentNode().name == customSpiderBackId) {
		// 其他禁用
		$("#openFile").css('display', 'none');
		$("#deleteFile").css('display', 'none');
		// 可新建
		...
		$("#installPackage").css('display', 'block');
			...
			&& treeNode.getParentNode().getParentNode() != null
			&& treeNode.getParentNode().getParentNode().getParentNode() == null
			&& treeNode.getParentNode().getParentNode().name == customSpiderBackId
			&& treeNode.name.endWith(".js")) {
		// 其他禁用
		$("#installPackage").css('display', 'none');
		$("#newFile").css('display', 'none');
		// 可打开，删除
		$("#openFile").css('display', 'block');
		$("#deleteFile").css('display', 'block');
	} else if (treeNode.getParentNode() != null
			&& treeNode.getParentNode().name == 'node_modules') {
		$("#installPackage").css('display', 'none');
		$("#newFile").css('display', 'none');
		$("#openFile").css('display', 'none');
		$("#deleteFile").css('display', 'block');
...
function newFile() {
	...
	// 0表示java文件，1表示python和2表示js
	var url = prefix + '/newFile/2/' + $('#customSpiderId').val() + "/"
			+ selectTreeNode.id;
...
function installPackage() {
	...
	// 安装依赖包
	// 0表示python文件，1表示js
	var url = prefix + '/installPackage/1/' + $('#customSpiderId').val() + "/"
			+ selectTreeNode.id;
	layer_showAuto("安装依赖包库", url);
}
...
function saveFile() {
	...
	if (fileContent == '') {
		$.modalAlert("无代码", modal_status.FAIL);
		return;
	}
	$.ajax({
		cache : true,
