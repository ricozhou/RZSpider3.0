...
function showRMenu(type, x, y, treeNode) {
	...
	if (treeNode.name == 'publiclib' || treeNode.name == 'Lib') {
		...
		$("#newFile").css('display', 'block');
			...
			&& treeNode.getParentNode().getParentNode().name != 'venv'
			&& treeNode.getParentNode().getParentNode().name != 'publiclib'
			&& treeNode.name != 'Scripts') {
		// 其他禁用
		$("#installPackage").css('display', 'none');
		$("#newFile").css('display', 'none');
		// 可打开，删除
		...
		$("#deleteFile").css('display', 'block');
			...
			&& treeNode.getParentNode().name == 'site-packages'
			&& treeNode.getParentNode().getParentNode().name != null) {
		// 可上传，下载,删除
...
function newFile() {
	...
	var url = prefix + '/newFile/1/' + $('#customSpiderId').val() + "/"
			+ selectTreeNode.id;
...
function installPackage() {
	...
	// 安装依赖包
	// 0表示python文件，1表示js
	var url = prefix + '/installPackage/0/' + $('#customSpiderId').val() + "/"
			+ selectTreeNode.id;
